...
export function MascotBubble({
  text, expression = 'idle', name, size = 72, growth, showMascot = true, reportable = false,
}: { text: string; expression?: Expression; name?: string; size?: number; growth?: number; showMascot?: boolean; reportable?: boolean }) {
  const c = useColors();
  return (
    <View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: space.s }}>
      {showMascot ? <Mascot size={size} expression={expression} growth={growth} /> : null}
